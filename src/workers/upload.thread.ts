...
import { UploadStateEnum } from "../collections2/idb/collections2Store.types";
...
export class UploadThreadWorker {
  ...
  async pauseJob(): Promise<boolean> {
    if (this.currentJob) {
      await updateUploadJobState(
        this.currentJob.uploadId,
  ...
  async _processJob() {
    ...
    try {
      ...
      if (!fuuid)
        throw new Error("File unique identifier (fuuid) not provided");
      if (!uploadUrl)
        throw new Error("No filehost url provided for the upload");
  ...
  async uploadFile(
    ...
    let processFilePromise = Promise.resolve().then(async () => {
      try {
        ...
        if (response.status !== 200) {
          throw new Error(
            "Unsupported response status for file uploda POST verification: " +
              response.status,
          );
        }
    ...
    if (!result) {
      console.info(
        "Timeout waiting for file verification, moving to next upload",
      );
  ...
  async uploadParts(
    ...
    while (true) {
      ...
      if (THROTTLE_UPLOAD)
        await new Promise((resolve) => setTimeout(resolve, THROTTLE_UPLOAD)); // Throttle
      try {
        await axios({
          method: "PUT",
          url: putUrl,
        ...
        if (signal.aborted) {
          if (!this.pauseUpload) {
            ...
            axios({
              ...
              console.warn(
                "Error sending cancel command to filehost for cancelled upload",
                err,
              ),
            );
          }
          ...
          if (axiosErr.status === 412) {
            ...
            const currentPositionStr =
              axiosErr.response?.headers["x-current-position"];
            const currentPositionInt = Number.parseInt(currentPositionStr);
            if (currentPositionInt) {
              console.info(
                "Resetting upload position from header value: %s",
                currentPositionInt,
              );
...
function progressEventHandler(
  ...
  job: UploadJobType,
  position: number,
  callback: UploadWorkerCallbackType,
) {
  let totalSize = job.size;
  let partPosition = e.loaded;
  let currentPosition = position + partPosition;
  callback(job.uploadId, job.userId, false, currentPosition, totalSize).catch(
    (err) => console.error("Error on progressEventHandler", err),
  );
}
