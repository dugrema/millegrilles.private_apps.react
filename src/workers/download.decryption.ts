...
import { DownloadIdbType } from "../collections2/idb/collections2Store.types";
import {
  removeDownload,
  saveDecryptionError,
  setDownloadJobComplete,
...
export class DownloadDecryptionWorker {
  ...
  async cancelJobIf(fuuid: string, userId: string): Promise<boolean> {
    if (
      this.currentJob?.fuuid === fuuid &&
      this.currentJob?.userId === userId
    ) {
      this.cancelled = true;
      return true;
    }
  ...
  async _decryptContent(downloadJob: DownloadIdbType) {
    ...
    if (!downloadJob.nonce)
      throw new Error("Decryption information (nonce) is missing");
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Error marking download for fuuid %s in error(2): %O, removing download",
          fuuid,
          err,
        );
   ...
   * @returns
   */
  async decryptInPlace(
    downloadJob: DownloadIdbType,
    fileHandle: FileSystemFileHandle,
  ) {
    // @ts-ignore
    if (!fileHandle.createSyncAccessHandle)
      throw new Error("Sync access to file not available");
    ...
    try {
      ...
      while (positionReading < fileSize) {
        if (decryptedPosition > positionReading)
          throw new Error("Overlap in reading/writing positions");
        if (this.cancelled) {
        ...
        if (cleartext) {
          let writeLen = syncFileHandle.write(cleartext, {
            at: decryptedPosition,
          });
      ...
      if (decryptedPosition > positionReading)
        throw new Error(
          "Error decrypting file - clear content longer than encrypted",
        );
  ...
  async decryptCopy(
    ...
    let outputFileHandle = await downloadDirectory.getFileHandle(
      downloadJob.fuuid + ".decrypted",
      { create: true },
    );
    // @ts-ignore
    let outputWriter = await outputFileHandle.createWritable({
