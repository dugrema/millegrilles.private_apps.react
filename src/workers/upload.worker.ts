...
export class AppsUploadWorker {
  ...
  async setup(
    stateCallback: UploadStateCallback,
    caPem: string,
    sharedMode: boolean,
  ) {
    ...
    if (!this.uploadWorker) {
      try {
        ...
        console.warn(
          "Error starting a Dedicated WebWorker, using direct instanciation",
          err,
        );
    ...
    if (!this.encryptionWorker) {
      ...
      await this.encryptionWorker.setup(
        this.encryptionStateCallbackProxy,
        caPem,
      );
    }
  ...
  async setEncryptionKeys(pems: Array<string[]>) {
    if (!this.encryptionWorker)
      throw new Error("Upload encryption worker not initialized");
  ...
  async uploadCallback(
    ...
    if (done) {
      ...
      this.triggerJobs().catch((err) =>
        console.error("uploadCallback Error on triggerJobs", err),
      );
      await this.triggerListChanged();
    } else {
      let upload = {
        workerType: UploadWorkerType.UPLOAD,
        uploadId,
        state: UploadStateEnum.UPLOADING,
        position,
        totalSize: size,
      } as UploadTransferProgress;
  ...
  async encryptionCallback(
    ...
    if (done) {
      ...
      // Start next download job (if any). Also does a produceState()
      this.triggerJobs().catch((err) =>
        console.error("encryptionCallback Error on triggerJobs", err),
      );
      await this.triggerListChanged();
    } else {
      let encryption = {
        workerType: UploadWorkerType.ENCRYPTION,
        uploadId,
        state: UploadStateEnum.ENCRYPTING,
        position,
        totalSize: size,
      } as UploadTransferProgress;
  ...
  async resumeUpload(uploadId: number) {
    ...
    if (!CONST_RESUMABLE_STATES.includes(job.state))
      throw new Error("Job not in a resumable state");
  ...
  async produceState() {
    ...
    this.stateCallback(update).catch((err) =>
      console.error("Error on upload produceState callback", err),
    );
  }
  ...
  maintain() {
    ...
    this.maintainCallbacks().catch((err) =>
      console.error("Error maintaining callbacks", err),
    );
  }
