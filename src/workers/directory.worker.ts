...
export class DirectoryWorker {
  ...
  async processDirectoryChunk(
    ...
    for (let file of mappedFiles) {
      ...
      if (file.comments) {
        ...
        for await (const commentWrapper of file.comments) {
          ...
          const decryptedContent = await encryption.decryptMessage(
            ...
            decryptionKey.cle_secrete_base64,
            encrypted_data.nonce,
            encrypted_data.ciphertext_base64,
          ...
          const decryptedComment = {
            comment_id: commentWrapper.comment_id,
            date,
            comment: decryptedContentValue.comment,
            tags: decryptedContentValue.tags,
            user_id,
          } as FileComment;
      ...
      if (encrypted) {
        ...
        let ref_hachage_bytes =
          //@ts-ignore
          encrypted.ref_hachage_bytes || (fuuid as string | null);
        ...
        if (keyId) {
          ...
          if (key) {
            ...
            if (format !== "mgs4") {
              console.warn(
                "Unsupported decryption format: %s - SKIPPING",
                format,
              );
              continue;
            }
            ...
            if (!nonce && encrypted.header) {
              // Legacy
              nonce = encrypted.header.slice(1); // Remove multibase 'm' marker
            }
            if (!nonce) {
              console.warn(
                "No format/nonce for metadata of file %s - SKIPPING",
                file.tuuid,
              );
              continue;
            }
            ...
            try {
              let secretKeyBytes = multiencoding.decodeBase64(
              ...
              console.error(
                "Error decrypting %s - SKIPPING. Err:\n%O",
                file.tuuid,
                err,
              );
            }
            ...
            console.warn(
              "File tuuid:%s, cleId:%s not provided",
              file.tuuid,
              keyId,
            );
          }
          ...
          console.warn(
            "File tuuid:%s, cleId:%s is not available",
            file.tuuid,
            keyId,
          );
        }
      ...
      try {
          ...
          if (keyId && thumbnail.data_chiffre) {
            ...
            if (key) {
              ...
              if (format === "mgs4") {
                ...
                console.warn(
                  "Unsupported encryption format for thumbnail (%s)",
                  format,
                );
              }
            } else {
              console.warn(
                "Key not provided to decrypt thumbnail for tuuid: %s",
                file.tuuid,
              );
            }
          } else {
            console.warn(
              "No key available to decrypt thumbnail for tuuid: %s",
              file.tuuid,
            );
          }
        }
        ...
        console.error(
          "Error decrypting thumbnail for tuuid %s: ",
          file.tuuid,
          err,
        );
      }
    ...
    if (!opts?.noidb) {
      let nonDeletedFiles = mappedFiles.filter(
        (item) => !deletedFiles.has(item.tuuid),
      );
  ...
  async authenticateFilehost(
    authenticationMessage: messageStruct.MilleGrillesMessage,
  ) {
    ...
    let authUrl = new URL(
      `https://${url.hostname}:${url.port}/filehost/authenticate`,
    );
  ...
  async openFile(
    ...
    decryptionInformation: messageStruct.MessageDecryption,
  ...
  async testFileSystem1(): Promise<string> {
    ...
    try {
      ...
      let test1Directory = await root.getDirectoryHandle("test1", {
        create: true,
      });
      ...
      let fileHandle = await test1Directory.getFileHandle("cfile.bin", {
        create: true,
      });
      ...
      let bytesWritten = syncHandle.write(new TextEncoder().encode("tada"), {
        at: 0,
      });
