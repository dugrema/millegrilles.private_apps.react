...
import {
  findDownloadPosition,
  removeDownloadParts,
  updateDownloadJobState,
} from "../collections2/idb/collections2StoreIdb";
import { DownloadStateEnum } from "../collections2/idb/collections2Store.types";
  ...
  async _processJob() {
    ...
    try {
      ...
      if (response.status === 206) {
        // Resuming
        console.debug(
          "Resuming download of fuuid:%s from position",
          currentJob.fuuid,
          positionOuter,
        );
      ...
      let statusCallback = (position: number, totalSize: number | null) => {
        ...
        if (callback && totalSize) {
          callback(
            currentJobLocal.fuuid,
            currentJobLocal.userId,
            false,
            position,
            totalSize,
          );
...
async function streamResponse(
  ...
  statusCallback: (position: number, totalSize: number | null) => void,
) {
  ...
  try {
    ...
    let fileHandle = await downloadDirectory.getFileHandle(job.fuuid, {
      create: true,
    });
    ...
    if (position > 0) {
      ...
      if (fileSize !== position)
        throw new Error("File position downloading and actual mismatch");
    }
    // @ts-ignore
    ...
    // @ts-ignore
      ...
      // @ts-ignore
      writeFileHandle = await fileHandle.createWritable({
