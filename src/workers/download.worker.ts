...
import {
  DownloadThreadWorker,
  DownloadWorkerCallbackType,
} from "./download.thread";
import {
  DecryptionWorkerCallbackType,
  DownloadDecryptionWorker,
} from "./download.decryption";
import {
  addDownload,
  getDownloadContent,
  getDownloadJob,
  getNextDecryptionJob,
  getNextDownloadJob,
  removeDownload,
  restartNextJobInError,
  updateDownloadJobState,
} from "../collections2/idb/collections2StoreIdb";
import {
  createDownloadEntryFromFile,
  createDownloadEntryFromVideo,
} from "../collections2/transferUtils";
import { FilehostDirType } from "./directory.worker";
import {
  DownloadStateUpdateSharedType,
  DownloadStateUpdateType,
...
export type DownloadStateCallback = (
  state: DownloadStateUpdateType,
) => Promise<void>;
...
export class AppsDownloadWorker {
  ...
  decryptionWorker:
    | Remote<DownloadDecryptionWorker>
    | DownloadDecryptionWorker
    | null;
  ...
  async downloadCallback(
    ...
    if (done) {
      ...
      let download = {
        workerType: DownloadWorkerType.DOWNLOAD,
        fuuid,
        state: DownloadStateEnum.DOWNLOADING,
        position,
        totalSize: size,
      } as DownloadTransferProgress;
      this.downloadStatus = download;
      await this.produceState();
    }
  }

  async decryptionCallback(
    fuuid: string,
    userId: string,
    done: boolean,
    position?: number | null,
    ...
    if (done) {
      ...
      let decryption = {
        workerType: DownloadWorkerType.DECRYPTION,
        fuuid,
        state: DownloadStateEnum.ENCRYPTED,
        position,
        totalSize: size,
      } as DownloadTransferProgress;
  ...
  async addDownloadFromFile(
    tuuid: string,
    userId: string,
  ): Promise<Blob | null> {
    if (!this.downloadWorker || !this.decryptionWorker)
      throw new Error("Dedicated workers not initialized");
  ...
  async pauseDownload(fuuid: string, userId: string) {
    ...
    const CONST_PAUSABLE_STATES = [
      DownloadStateEnum.INITIAL,
      DownloadStateEnum.DOWNLOADING,
    ];
    if (!CONST_PAUSABLE_STATES.includes(job.state))
