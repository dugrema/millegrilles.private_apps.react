import { lazy, ChangeEvent, useCallback, useEffect, useMemo, useState, Fragment, Dispatch } from "react";
import { useParams, Link } from "react-router-dom";
import Datetime from 'react-datetime';

import useWorkers from "../workers/workers";
import useSenseursPassifsStore, { DeviceReadings, DeviceReadingValue } from "./senseursPassifsStore";
import { SenseursPassifsStatistiquesItem, SenseursPassifsStatistiquesResponse, StatisticsRequestType } from "../workers/connection.worker";
    ...
    return (
        <>
            <nav>
                ...
                <Link to={`/apps/senseurspassifs/device/${deviceId}`}
                    className='btn inline-block text-center bg-slate-700 hover:bg-slate-600 active:bg-slate-500'>
                        Device
                </Link>
            </nav>
            ...
            <StatistiquesSenseur device={device} componentId={componentId} component={component} />
        </>
    )

}

function StatistiquesSenseur(props: {device: DeviceReadings, componentId: string, component: DeviceReadingValue}) {
    ...
    let [data, setData] = useState(null as SenseursPassifsStatistiquesResponse | null);

    let groupingHandler = useCallback((e: ChangeEvent<HTMLSelectElement>)=>setGrouping(e.currentTarget.value), [setGrouping]);
        ...
        <div>
            ...
            <div className="grid grid-cols-12">
                ...
                <div className="col-span-10">
                    <select value={grouping} onChange={groupingHandler}
...
function FormatterValeur(props: {value: any, valueType: string, hideType?: boolean}) {
    ...
    if(value !== null && decimals !== null) {
        return <span>{value.toFixed(decimals)} {unite}</span>
    } else {
...
function getUnite(typeValeur: string) {
    ...
    switch(typeValeur) {
        ...
        case 'humidite': decimals = 1; unite = <span>%</span>; break
        case 'pression': decimals = 0; unite = <span> hPa</span>; break
        case 'pression_tendance': decimals = 0; unite = <span> Pa</span>; break
        default:
    }
    return [decimals, unite]
}

function StatistiquesTable72h(props: {data?: Array<SenseursPassifsStatistiquesItem>, valueType: string}) {
...
function StatistiquesTable31j(props: {data?: Array<SenseursPassifsStatistiquesItem>, valueType: string}) {
    const {data, valueType} = props
...
function HoursList(props: {data: Array<SenseursPassifsStatistiquesItem>, valueType: string}) {
    ...
    let elems = data.map(item=>{
        ...
        return (
            <Fragment key={''+item.heure}>
                ...
                <div className='col-span-1'><Formatters.FormatterDate format='HH:mm:ss' value={item.heure} /></div>
                <div className='col-span-1 text-right'><FormatterValeur value={item.avg} valueType={valueType} hideType={true} /></div>
                <div className='col-span-1 text-right'>
                    {afficherMax?
                        <FormatterValeur value={item.max} valueType={valueType} hideType={true} />
                        :''
                    }
                </div>
                <div className='col-span-1 text-right'>
                    {afficherMin?
                        <FormatterValeur value={item.min} valueType={valueType} hideType={true} />
                        :''
                    }
                </div>
...
function DaysList(props: {data: Array<SenseursPassifsStatistiquesItem>, valueType: string}) {
    const { data, valueType } = props;
    ...
    let elems = data.map((item, idx)=>(
        <Fragment key={''+item.heure}>
            <div className='col-span-2'><Formatters.FormatterDate value={item.heure} format="YYYY/MM/DD" /></div>
            <div className='col-span-1 text-right'><FormatterValeur value={item.avg} valueType={valueType} hideType={true} /></div>
            <div className='col-span-1 text-right'>
                {afficherMax?
                    <FormatterValeur value={item.max} valueType={valueType} hideType={true} />
...
function StatistiquesTableCustom(props: StatistiquesTableCustomProps) {
    ...
    const FormatteurListe = useMemo(()=>{
        ...
        switch(grouping) {
            case 'jours': Formatteur = DaysList; break;
            default:
