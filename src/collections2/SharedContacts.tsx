...
import { Collection2ContactItem } from "../types/connection.types";
...
function CurrentContactList() {
  ...
  let usernameOnChange = useCallback(
  ...
  useEffect(() => {
    ...
    workers.connection
      .getCollection2ContactList()
      .then((response) => setContacts(response.contacts))
      .catch((err) => console.error("Error loading contacts", err));
  }, [workers, ready]);

  let contactDeleteHandler = useCallback(
    async (e: MouseEvent<HTMLButtonElement>) => {
      ...
      let response =
        await workers.connection.deleteCollection2Contact(contactId);
      if (!response.ok)
        throw new Error("Error deleted share user: " + response.err);
...
export function sortContacts(
  a: Collection2ContactItem,
  b: Collection2ContactItem,
) {
  if (a === b) return 0;
