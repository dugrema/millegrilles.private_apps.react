...
export function Modals(props: ModalProps) {
  ...
  if (modal === ModalEnum.NewDirectory)
    return (
      <ModalNewDirectory
        workers={workers}
        ready={ready}
  ...
  if (modal === ModalEnum.Share)
    return (
      <ModalShareCollection
        workers={workers}
        ready={ready}
        modalType={modal}
        close={closeModal}
...
export function ModalInformation(
  props: ModalInformationProps & { shared?: boolean | null },
) {
  ...
  let [statsSubdirectories, setStatsSubdirectories] = useState(
    null as Collection2DirectoryStats[] | null,
  );
  ...
  useEffect(() => {
    ...
    workers.connection
      .getCollection2Statistics(cuuid)
      .then((response) => {
      ...
      <div className="relative p-4 w-full max-w-2xl max-h-full">
        <div className="relative rounded-lg shadow bg-gray-800">
          <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-gray-600">
            ...
            <button
              onClick={close}
              className="bg-transparent rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center hover:bg-gray-600 hover:text-white"
            >
              <CloseIcon />
            ...
            <div className="grid grid-cols-3">
              {statsValues ? (
                <>
                  ...
                  <div className="col-span-2">
                    <Formatters.FormatteurTaille
                      value={statsValues.totalSize}
                    />
                  </div>
                  <div className="text-base leading-relaxed text-gray-400">
                    Files
...
export function ModalNewDirectory(props: ModalInformationProps) {
  ...
  let directoryNameOnChange = useCallback(
    (e: ChangeEvent<HTMLInputElement>) =>
      setDirectoryName(e.currentTarget.value),
    [setDirectoryName],
  );

  let actionHandler = useCallback(
    async (
      e?: MouseEvent<HTMLButtonElement>,
      opt?: { skipTimeout?: boolean },
    ) => {
      ...
      let response = await workers.connection.addDirectoryCollection2(
        result.command,
        result.key,
      );
      if (!response.ok)
        throw new Error("Error creating new directory: " + response.err);
          ...
          <div className="p-4 md:p-5 space-y-4">
            ...
            <form onSubmit={submitHandler}>
              <input
                ...
                onChange={directoryNameOnChange}
                autoFocus
              />
            </form>
          </div>
...
export function ModalRenameFile(props: ModalInformationProps) {
  ...
  let [selectedFile, isFile] = useMemo(() => {
    ...
    if (!selection || selection.length !== 1)
      throw new Error("Only 1 file can be renamed");
  ...
  let nameOnChange = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => setNewName(e.currentTarget.value),
    [setNewName],
  );
  let [newMimetype, setNewMimetype] = useState(selectedFile.mimetype || "");
  let mimetypeOnChange = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => setNewMimetype(e.currentTarget.value),
    [setNewMimetype],
  );
  let [error, setError] = useState(false);
  ...
  let actionHandler = useCallback(
    async (
      ...
      if (isFile && newMimetype.length === 0)
      ...
      if (changed) {
        ...
        if (!fileIdb || !fileIdb.secretKey || !fileIdb.decryptedMetadata)
          throw new Error("File detail not available locally");
        let { decryptedMetadata, secretKey } = fileIdb;
        let cleId = fileIdb.keyId;
        if (!cleId)
          throw new Error(
            "Insufficient information to re-encrypt data for tuuid: " + tuuid,
          );

        let updatedMetadata = { ...decryptedMetadata };
        updatedMetadata.nom = newName;
        let reencryptedMetadata = await updateEncryptedContent(
          workers,
          cleId,
          secretKey,
          updatedMetadata,
        ...
        console.debug(
          "Updated file/directory metadata: %O, encrypted: %O",
          updatedMetadata,
          reencryptedMetadata,
        );

        if (isFile) {
          let result = await workers.connection.renameFileCollection2(
            tuuid,
            reencryptedMetadata,
            newMimetype,
          );
          ...
          let result = await workers.connection.renameDirectoryCollection2(
            tuuid,
            reencryptedMetadata,
          );
          if (!result.ok)
            throw new Error("Error updating directory: " + result.err);
        }
      }
          ...
          <div className="p-4 md:p-5">
            <form onSubmit={submitHandler}>
              ...
              <label
                ...
                className="text-base leading-relaxed text-gray-400 block pt-4"
              >
                New name
              </label>
              <input
...
export function ModalBrowseAction(
  props: ModalInformationProps & {
    title: string;
    shared?: boolean | null;
    includeDeleted?: boolean | null;
  },
  ...
  let filesDict = useUserBrowsingStore(
    (state) => state.modalNavCurrentDirectory,
  );
  ...
  let files = useMemo(() => {
    ...
    let filesValues = Object.values(filesDict).filter(
      (item) => item.type_node !== "Fichier",
    );
    return filesValues;
  }, [filesDict]) as TuuidsBrowsingStoreRow[] | null;

  let onClickRow = useCallback(
    (e, tuuid, typeNode, range) => {
  ...
  let actionHandler = useCallback(async () => {
    ...
    if (!cuuid && !includeDeleted)
      throw new Error("Cannot move/copy from root");
    if (!modalNavCuuid)
      throw new Error("Must select a directory (root not valid)");
    ...
    if (modalType === ModalEnum.Copy) {
      ...
      let response = await workers.connection.copyFilesCollection2(
        modalNavCuuid,
        selection,
        opts,
      );
      ...
      let response = await workers.connection.moveFilesCollection2(
        cuuid,
        modalNavCuuid,
        selection,
      );
...
function ContactRow(props: {
  contact: Collection2ContactItem;
  checked: boolean;
  ...
  return (
    <div
      onClick={onClick}
      data-contactid={contact.contact_id}
...
export function ModalShareCollection(props: ModalInformationProps) {
  ...
  let [contacts, setContacts] = useState(
    null as Collection2ContactItem[] | null,
  );
  let [sharedWithContactIds, setSharedWithContactIds] = useState(
    [] as string[],
  );
  let [initialSharedWithContactIds, setInitialSharedWithContactIds] = useState(
    null as string[] | null,
  );
  ...
  let actionHandler = useCallback(async () => {
    if (!workers || !ready || !sharedWithContactIds)
      throw new Error("Workers not initialized or value empty");
    if (!selection || selection.length === 0)
      throw new Error("At least 1 collection must be selected");

    if (sharedWithContactIds.length > 0) {
      let response = await workers.connection.shareCollection2Collection(
        selection,
        sharedWithContactIds,
      );
      if (!response.ok)
        throw new Error("Error changing shared collections: " + response.err);
    }
    ...
    if (initialSharedWithContactIds) {
      ...
      for (let cuuid of selection) {
        for (let contactId of toDelete) {
          let response =
            await workers.connection.removeShareCollection2Collection(
              cuuid,
              contactId,
            );
          if (!response.ok)
            throw new Error("Error removing contact: " + response.err);
        }
      }
    }
  ...
  useEffect(() => {
    ...
    if (!selection || selection.length < 1)
      throw new Error(
        "Selection invalid - selection must have at least 1 element",
      );

    // Check that all items in the selection are directories
    let selectedItems = selection
      //@ts-ignore
      .map((item) => currentDirectory[item])
      .filter((item) => item?.type_node !== "Fichier");
    if (selectedItems.length !== selection.length)
      throw new Error(
        "At least one selected item is not a collection/directory",
      );
    ...
    Promise.resolve()
      .then(async () => {
        ...
        let contactsResponse =
          await workers.connection.getCollection2ContactList();
        ...
        let sharedCollectionsResponse =
          await workers.connection.getCollection2SharedCollections();
        if (sharedCollectionsResponse.partages) {
          ...
          if (selection && selection.length === 1) {
            ...
            sharedContactIds = sharedCollectionsResponse.partages
              .filter((item) => item.tuuid === cuuid)
              .map((item) => {
          ...
          let sharedCollections = sharedContactIds.filter(
            (item) => item,
          ) as string[];
      ...
      .catch((err) =>
        console.error("Error loading contacts / shared collections", err),
      );
  }, [
    workers,
    ready,
    ...
    currentDirectory,
    setContacts,
    setInitialSharedWithContactIds,
    setSharedWithContactIds,
  ]);
