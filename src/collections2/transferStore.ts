...
export type DownloadTransferProgress = {
  workerType: DownloadWorkerType;
  fuuid: string;
...
interface TransferStoreState {
  ...
  setDownloadTicker: (
    downloadActivity: TransferActivity,
    downloadTransferPercent: number | null,
    opts?: { states: { [state: number]: number } },
  ) => void;
...
const useTransferStore = create<TransferStoreState>()(
  devtools((set) => ({
    ...
    setDownloadTicker: (downloadActivity, downloadTransferPercent, opts) =>
      set(() => {
        let values = {
          downloadActivity,
          downloadTransferPercent,
        } as TransferStoreState;
        if (opts?.states) {
          values.downloadSummary = opts.states;
        }
        return values;
      }),
    setDownloadSessionStart: (downloadSessionStart) =>
      set(() => ({ downloadSessionStart })),
    updateDownloadState: (updatedState) =>
      set((state) => {
        ...
        if (updatedState.activeTransfers) {
          ...
          if (!state.downloadSessionStart)
            values.downloadSessionStart = new Date();
    ...
    setDownloadJobsDirty: (downloadJobsDirty) =>
      set(() => ({ downloadJobsDirty })),

    setUploadTicker: (uploadActivity, uploadTransferPercent, opts) =>
      set(() => {
        let values = {
          uploadActivity,
    ...
    setUploadSessionStart: (uploadSessionStart) =>
      set(() => ({ uploadSessionStart })),
    updateUploadState: (updatedState) =>
      set((state) => {
