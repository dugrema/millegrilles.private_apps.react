...
function UserFileViewing() {
  ...
  let breacrumbOnClick = useCallback(
    (tuuid: string | null) => {
      if (tuuid) {
  ...
  const updateFileHandler = useCallback(async () => {
    ...
    if (response.files?.length === 1 && response.keys?.length === 1) {
      ...
      throw new Error(
        `Error loading file, mising content or key for tuuid:${tuuid}`,
      );
    }
  }, [workers, ready, tuuid]);
  ...
  const deleteCommentHandler = useCallback(
    async (e: MouseEvent<HTMLButtonElement>) => {
      ...
      const response = await workers.connection.deleteCollection2Comment(
        tuuid,
        commentId,
      );
      if (response.ok !== true)
        throw new Error(`Error deleting comment: ${response.err}`);
  ...
  useEffect(() => {
    ...
    updateFileHandler().catch((err) =>
      console.error("Error loading file %s: %O", tuuid, err),
    );
...
function Breadcrumb(props: BreadcrumbProps) {
  ...
  let breadcrumbMapped = useMemo(() => {
    ...
    let fileElem = (
      <div
        key={file.tuuid}
        className="inline items-center pl-2 text-sm bg-slate-700 bg-opacity-50 font-bold pr-2"
      >
