...
import { removeUserUploads } from "./idb/collections2StoreIdb";
...
import { UploadStateEnum } from "./idb/collections2Store.types";
      ...
      <section className="fixed top-10">
        ...
        <div>
          <ActionButton
            ...
            mainButton={true}
            disabled={!completedTransfersPresent}
            revertSuccessTimeout={3}
          >
            Remove completed
          </ActionButton>
          <ActionButton
            onClick={pauseUploadingHandler}
            revertSuccessTimeout={3}
          >
...
function UploadSummary() {
  ...
  let summaryList = useMemo(() => {
    ...
    let queued =
      uploadSummary[UploadStateEnum.INITIAL] +
      uploadSummary[UploadStateEnum.GENERATING] +
      uploadSummary[UploadStateEnum.SENDCOMMAND] +
      uploadSummary[UploadStateEnum.READY] +
      uploadSummary[UploadStateEnum.UPLOADING] +
      uploadSummary[UploadStateEnum.VERIFYING] +
    ...
    list.push(
      <React.Fragment key="done">
        <p>Done</p>
        <p className="text-right pr-1">{uploadSummary[UploadStateEnum.DONE]}</p>
      </React.Fragment>,
    );
    list.push(
      <React.Fragment key="pause">
        <p>Paused</p>
        <p className="text-right pr-1">
          {uploadSummary[UploadStateEnum.PAUSED]}
        </p>
      </React.Fragment>,
    );
    list.push(
      <React.Fragment key="initial">
        <p>Queued</p>
        <p className="text-right pr-1">{queued}</p>
      </React.Fragment>,
    );
    list.push(
      <React.Fragment key="error">
        <p>Error</p>
        <p className="text-right pr-1">
          {uploadSummary[UploadStateEnum.ERROR]}
        </p>
      </React.Fragment>,
    );
    ...
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        ...
        {typeof encryptionPercent === "number" ? (
          <div>
            <ProgressBar value={encryptionPercent} />
          </div>
        ) : (
          <>None</>
        )}
...
function OngoingTransfers() {
  ...
  let mappedTransfers = useMemo(() => {
    let jobs = uploadJobs?.filter(
      (item) => item.state !== UploadStateEnum.DONE,
    );
    if (!jobs) return [];
...
function CompletedTransfers() {
  ...
  let mappedTransfers = useMemo(() => {
    let completedJobs = uploadJobs?.filter(
      (item) => item.state === UploadStateEnum.DONE,
    );
...
const CONST_RESUME_STATE_CANDIDATES = [
  UploadStateEnum.PAUSED,
  UploadStateEnum.ERROR_DURING_PART_UPLOAD,
];
const CONST_PAUSE_STATE_CANDIDATES = [
  UploadStateEnum.READY,
  UploadStateEnum.UPLOADING,
];
...
function JobRow(props: JobRowProps) {
  ...
  let fullpath = useMemo(() => {
    ...
    filepath.push(
      <span key="filename" className="font-bold">
        {value.filename}
      </span>,
    );
    return filepath;
  ...
  let disablePauseButton = useMemo(
    () => !CONST_PAUSE_STATE_CANDIDATES.includes(value.state),
    [value],
  );
  let disableResumeButton = useMemo(
    () => !CONST_RESUME_STATE_CANDIDATES.includes(value.state),
    [value],
  );
  let rowBgCss = useMemo(() => {
    if (
      [UploadStateEnum.ENCRYPTING, UploadStateEnum.UPLOADING].includes(
        value.state,
      )
    )
      return "odd:bg-violet-700 even:bg-violet-600";
    if ([UploadStateEnum.ERROR].includes(value.state))
      return "odd:bg-red-700 even:bg-red-600";
    if (
      [
        UploadStateEnum.ERROR_DURING_PART_UPLOAD,
        UploadStateEnum.PAUSED,
      ...
      return "odd:bg-yellow-800 even:bg-yellow-700";
  ...
  return (
    <div
      key={value.uploadId}
      className={`grid grid-cols-6 ${rowBgCss} odd:bg-opacity-40 even:bg-opacity-40 hover:bg-violet-800 gap-x-1 px-2 py-1`}
    >
      <Link to={`/apps/collections2/b/${value.cuuid}`} className="col-span-3">
        {fullpath}
      </Link>
      <Formatters.FormatteurTaille
        value={value.clearSize || value.size || undefined}
      />
