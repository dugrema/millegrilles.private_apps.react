...
import { DownloadStateEnum } from "./idb/collections2Store.types";
...
function DownloadSummary() {
  ...
  let summaryList = useMemo(() => {
    ...
    let queued =
      downloadSummary[DownloadStateEnum.INITIAL] +
      downloadSummary[DownloadStateEnum.DOWNLOADING] +
      downloadSummary[DownloadStateEnum.ENCRYPTED];
    ...
    list.push(
      <React.Fragment key="done">
        ...
        <p className="text-right pr-1">
          {downloadSummary[DownloadStateEnum.DONE]}
        </p>
      </React.Fragment>,
    );
    list.push(
      <React.Fragment key="pause">
        <p>Paused</p>
        <p className="text-right pr-1">
          {downloadSummary[DownloadStateEnum.PAUSED]}
        </p>
      </React.Fragment>,
    );
    list.push(
    ...
    list.push(
      <React.Fragment key="error">
        <p>Error</p>
        <p className="text-right pr-1">
          {downloadSummary[DownloadStateEnum.ERROR]}
        </p>
      </React.Fragment>,
    );
    ...
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        ...
        {typeof decryptionPercent === "number" ? (
          <div>
            <ProgressBar value={decryptionPercent} />
          </div>
        ) : (
          <>None</>
        )}
...
function OngoingTransfers() {
  ...
  let mappedTransfers = useMemo(() => {
    let jobs = downloadJobs?.filter((item) =>
      CONST_ONGOING_STATES.includes(item.state),
    );
    if (!jobs) return [];
...
function CompletedTransfers() {
  ...
  let mappedTransfers = useMemo(() => {
    let completedJobs = downloadJobs?.filter(
      (item) => item.state === DownloadStateEnum.DONE,
    );
...
function JobRow(props: JobRowProps) {
  ...
  let rowBgCss = useMemo(() => {
    if (
      [DownloadStateEnum.ENCRYPTED, DownloadStateEnum.DOWNLOADING].includes(
        value.state,
      )
    )
      return "odd:bg-violet-700 even:bg-violet-600";
    if ([DownloadStateEnum.ERROR].includes(value.state))
      return "odd:bg-red-700 even:bg-red-600";
    if ([DownloadStateEnum.PAUSED].includes(value.state))
  ...
  return (
    ...
    >
      <Link to={`/apps/collections2/f/${value.tuuid}`} className="col-span-3">
        {value.filename}
      </Link>
      ...
      <div>
        ...
        {onPause ? (
          <ActionButton
            ...
            varwidth={10}
            revertSuccessTimeout={3}
            disabled={!CONST_PAUSE_STATE_CANDIDATES.includes(value.state)}
          >
            <img
              src={PauseIcon}
        ...
        {onResume ? (
          <ActionButton
            ...
            varwidth={10}
            revertSuccessTimeout={3}
            disabled={!CONST_RESUME_STATE_CANDIDATES.includes(value.state)}
          >
            <img
              src={ForwardIcon}
