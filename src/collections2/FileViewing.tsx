...
export function DetailFileViewLayout(props: {
  ...
  let setLastOpenedFile = useUserBrowsingStore(
    (state) => state.setLastOpenedFile,
  );
  ...
  let isMedia = useMemo(() => {
    ...
    if (mimetype)
      return isVideoMimetype(mimetype) || supportsAudioFormat(mimetype);
    return false;
  }, [file, thumbnail]);

  // Set this file as the last opened. Used to highlight the file when going back to containing directory.
  useEffect(
    () => setLastOpenedFile(file?.tuuid || null),
    [file, setLastOpenedFile],
  );
...
function FileMediaLayout(
  props: FileViewLayoutProps & {
    thumbnail: Uint8Array | null;
    setLoadProgress: Dispatch<number | null>;
  },
) {
  let {
    file,
  ...
  let conversionScreenOpen = useCallback(
    () => setViewConversionScreen(true),
    [setViewConversionScreen],
  );
  let conversionScreenClose = useCallback(
    () => setViewConversionScreen(false),
    [setViewConversionScreen],
  );
  ...
  useEffect(() => {
    ...
    if (images) {
      if (file.fileData?.anime && file.fileData.mimetype?.startsWith("image")) {
        ...
        if (fuuids_versions && fuuids_versions.length > 0) {
          ...
          if (secretKey && nonce && format) {
            workers.directory
              .openFile(fuuid, secretKey, { nonce, format })
              .then((imageBlob) => {
      ...
      let maxImage = Object.values(images).reduce(
        (acc, item) => {
          if (!acc?.resolution || acc?.resolution < item.resolution)
            return item;
          return acc;
        },
      ...
      if (maxImage) {
        ...
        workers.directory
          .openFile(fuuid, secretKey, maxImage)
          .then((imageBlob) => {
            let imageBlobUrl = URL.createObjectURL(imageBlob);
            setFullSizeBlobUrl(imageBlobUrl);
          })
  ...
  if (!file) return <></>;
  if (viewConversionScreen)
    return <VideoConversion file={file} close={conversionScreenClose} />;
...
function MediaContentDisplay(
  props: FileViewLayoutProps & {
    ...
    setLoadProgress: Dispatch<number | null>;
    setVideoError: (code: number) => void;
  },
) {
  let {
    file,
  ...
  let [isVideoFile, isAudioFile, isPdf] = useMemo(() => {
    ...
    if (file?.fileData?.video) {
      ...
      else if (supportsVideoFormat(mimetype))
        return [true, true, false]; // Return true for video and audio
  ...
  useEffect(() => {
    ...
    if (videoFuuid) {
      ...
      if (fuuid === videoFuuid) {
        ...
        let video = Object.values(videos)
          .filter((item) => item.fuuid_video === videoFuuid)
          .pop();
        if (video) {
          // Found match
    ...
    let userMaxResolutionConfig = localStorage.getItem(
      CONST_VIDEO_MAX_RESOLUTION,
    ) as string;
    ...
    if (fuuid && mimetype) {
      ...
      if (file.fileData?.width && file.fileData?.height) {
        originalResolution = Math.min(
          file.fileData.width,
          file.fileData.height,
        );
      } else {
        originalResolution =
          file?.fileData?.width || file?.fileData?.height || null;
      }
    ...
    let video = Object.values(videos).reduce(
      (previous, item) => {
        let resolutionPrevious = null as number | null,
          resolutionCurrent = null as number | null;
        if (previous && previous.width && previous.height)
          resolutionPrevious = Math.min(previous.width, previous.height);
        if (item && item.width && item.height)
          resolutionCurrent = Math.min(item.width, item.height);
  ...
  useEffect(() => {
    ...
    workers.connection
      .getStreamingJwt(fuuidVideo, fuuidRef, contactId)
      .then((response) => {
...
function FileViewLayout(props: FileViewLayoutProps) {
  let { file, selectedVideo, setSelectedVideo, loadProgress, videoError } =
    props;
...
function FileDetail(
  ...
  let [fuuid, lastPresence, recentVisits] = useMemo(() => {
    ...
    if (visits) {
      recentVisits = Object.values(visits).filter(
        (item) => item > weekExpired,
      ).length;
  ...
  const canOpen = useMemo(() => {
    ...
    return (
      CONST_SUPPORTED_OPEN_TYPES.includes(mimetype) ||
  ...
  const openFileHandler = useCallback(async () => {
    ...
    if (!fuuid || !nonce || !format)
      throw new Error("Insufficient information to decrypt file");
    if (secretKey && nonce && format) {
      const mimetype = (file.fileData?.mimetype || "").toLocaleLowerCase();
      const fileBlob = await workers.directory.openFile(
        fuuid,
        secretKey,
        { nonce, format },
        mimetype,
  ...
  return (
    <div className="grid grid-cols-6 md:grid-cols-1">
      ...
      <p className="col-span-4" title={fileSize + " bytes"}>
        <Formatters.FormatteurTaille value={fileSize || undefined} />
      </p>
      <p className="col-span-2 text-slate-400">File date</p>
      <p className="col-span-4">
        <Formatters.FormatterDate
          value={file.decryptedMetadata?.dateFichier || file.date_creation}
        />
      </p>
      ...
      <p className="col-span-4">
        <Formatters.FormatterDate value={lastPresence || undefined} />
      </p>
      ...
      <div className="col-span-6">
        {canOpen ? (
          <ActionButton
            ...
            disabled={!ready && !fileBlob}
            revertSuccessTimeout={1}
          >
            Open
          </ActionButton>
        ) : (
...
function VideoPlayer(props: VideoPlayerProps) {
  ...
  let onTimeUpdate = useCallback(
    (e: ChangeEvent<HTMLVideoElement>) => {
      let currentTime = e.target.currentTime;
      if (!tuuid || !userId) return; // Missing information
      setVideoPosition(tuuid, userId, currentTime).catch((err) =>
        console.warn("Error updating video position", err),
      );
    },
    [tuuid, userId],
  );
  ...
  let onEnded = useCallback(
    (e: ChangeEvent<HTMLVideoElement>) => {
      if (!tuuid || !userId) return; // Missing information
      removeVideoPosition(tuuid, userId).catch((err) =>
        console.warn("Error removing video position", err),
      );
    },
    [tuuid, userId],
  );
  ...
  let videoClassname = useMemo(() => {
    ...
    if (fullscreen)
      return `grow object-contain object-center ${classNameProps}`;
...
export type FileVideoDataWithItemKey = FileVideoData & {
  entryKey: string;
  resolution: number;
};

export function sortVideoEntries(
  a: FileVideoDataWithItemKey,
  b: FileVideoDataWithItemKey,
): number {
  if (a === b) return 0;
...
function VideoSelectionDetail(
  props: FileViewLayoutProps & { file: TuuidsIdbStoreRowType } & {
    setSelectedVideo: Dispatch<FileVideoData | null>;
  },
) {
  let { file, selectedVideo, setSelectedVideo, loadProgress, videoError } =
  ...
  let onClickHandler = useCallback(
    (e: MouseEvent<HTMLLIElement>) => {
      ...
      let videos = file?.fileData?.video;
      if (value !== "original" && !videos)
        throw new Error("No video information to select");
      videos = videos || {};
  ...
  let elems = useMemo(() => {
    ...
    for (let videoItem of videoItems) {
      ...
      values.push(
        <li
          key={videoItem.fuuid_video}
          data-key={videoItem.entryKey}
          onClick={onClickHandler}
          className={
            "pl-2 hover:bg-violet-500 " +
            (selected ? "bg-violet-800 font-bold" : "")
          }
        >
          <Link to={url}>
            {videoItem.resolution}
            {videoItem.codec ? (
              <span className="pl-1">{videoItem.codec}</span>
            ) : (
              <></>
            )}
      ...
      if (support) {
        ...
        values.push(
          <li
            ...
            className={
              "pl-2 hover:bg-violet-500 " +
              (originalSelected ? "bg-violet-800 font-bold" : "")
            }
          >
  ...
  return (
    <>
      <p className="col-span-6 pt-4 text-slate-400">
        Selected video resolution
      </p>
      <ol className="col-span-6 cursor-pointer items-pl-2 max-w-48">{elems}</ol>
      <p
        className={
          "col-span-2 md:col-span-1 text-slate-400 duration-700 transition-all" +
          progressClassHide
        }
      >
        Loading progress
      ...
      <div
        className={
          "col-span-4 md:col-span-5 transition-all" + progressClassHide
        }
      >
        {videoError ? (
...
function AddComment(props: FileAddProps) {
  ...
  const commentOnChange = useCallback(
    (e: ChangeEvent<HTMLTextAreaElement>) => setComment(e.currentTarget.value),
    [setComment],
  );
  ...
  const addHandler = useCallback(async () => {
    ...
    const response = await workers.connection.collection2AddFileComment(
      file.tuuid,
      encryptedComment,
    );
    if (response.ok !== true)
      throw new Error("Error adding comment: " + response.err);
...
type FileCommentsProps = {
  file: TuuidsIdbStoreRowType | null;
  deleteHandler: (commentId: MouseEvent<HTMLButtonElement>) => Promise<void>;
};
...
function FileComments(props: FileCommentsProps) {
  ...
  const elems = sortedComments.map((item, idx) => {
    ...
    return (
      <div
        key={item.comment_id}
        className="grid grid-cols-3 lg:grid-cols-12 mb-4 hover:bg-violet-600/50"
      >
