...
import { Collection2DirectoryStats } from "../types/connection.types";
...
function BrowsingDeleted() {
  ...
  // Data loader
  let {
    data: deletedFilesPage,
    error,
    isLoading,
  } = useGetDeletedFiles({ userId, page: pageNo, cuuid: tuuid });
  console.debug(
    "Data loader data: %O, error: %O, isLoading: %s",
    deletedFilesPage,
    error,
    isLoading,
  );
  ...
  let onClickBreadcrumb = useCallback(
    (tuuid?: string | null) => {
      setTuuid(tuuid || null);
      changePage(1);
    },
  ...
  return (
    <>
      <section className="fixed top-12">
        <Breadcrumb
          breadcrumb={deletedFilesPage?.breadcrumb}
          onClick={onClickBreadcrumb}
        />
        <div className="pt-2">
          <ButtonBar
            onModal={onModal}
            inSubdirectory={!!tuuid}
            deletedFiles={deletedFilesPage}
          />
        </div>
      </section>
      ...
      <section className="fixed top-32 left-0 right-0 px-2 bottom-10 overflow-y-auto w-full">
        ...
        {isLoading ? (
          ...
          <FilelistPane
            files={deletedFilesPage?.list}
            sort={sortKey}
            dateColumn="modification"
...
export function ButtonBar(props: ButtonBarProps) {
  ...
  let selection = useUserBrowsingStore((state) => state.selection);
  let deleteFilesDirectory = useUserBrowsingStore(
    (state) => state.deleteFilesDirectory,
  );
  ...
  let recycleHandler = useCallback(async () => {
    ...
    if (!response.ok)
      throw new Error("Error deleting files/directories: " + response.err);
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 pt-1">
      <div className="col-span-2">
        <button
          ...
          className={
            "varbtn px-1 py-1 w-10 hover:bg-slate-600 active:bg-slate-500 " +
            (selectionMode ? "bg-violet-500" : "bg-slate-700")
          }
        >
          <img
            src={SelectionModeIcon}
            alt="Select files"
            title="Select files"
        ...
        <ActionButton
          ...
          disabled={!selectionMode || !selectCount || inSubdirectory}
          confirm={true}
          revertSuccessTimeout={2}
          varwidth={10}
        >
          <img
            src={RecycleIcon}
        ...
        <button
          onClick={copyHandler}
          disabled={!selectionMode || !selectCount}
...
function useGetDeletedFiles(params: FetcherParams): UseGetDeletedFilesType {
  ...
  let [fetcherKey, fetcherFunction] = useMemo(() => {
    let fetcherFunction = async (params: FetcherParams) =>
      fetchDeletedFilesPage(workers, ready, params);
...
async function fetchDeletedFilesPage(
  workers: AppWorkers | null,
  ready: boolean,
  params: FetcherParams,
): Promise<FetchDeteledFilesPageReturnType | null> {
  ...
  let response = await workers.connection.syncDeletedFiles(
    startIdx,
    params.cuuid,
    CONST_PAGE_SIZE,
  );
  ...
  if (responseBreadcrumb) {
    let decryptedBreadcrumb = await workers.directory.processDirectoryChunk(
      workers.encryption,
      userId,
      responseBreadcrumb,
      response.keys,
...
function Breadcrumb(props: BreadcrumbProps) {
    ...
    return breadcrumbMapped.map((item, idx) => {
      if (idx === lastIdx) {
        return (
          <div
            key={item.tuuid}
            className="inline pl-1 md:pl-2 text-sm bg-slate-700 bg-opacity-50 font-bold pr-2"
          >
            {item.nom}
          </div>
        );
        ...
        return (
          ...
          >
            <span onClick={onClickHandler} data-tuuid={item.tuuid}>
              {item.nom}
            </span>
  ...
  return (
    <nav
      ...
      className="w-screen leading-3 pr-2 line-clamp-2"
    >
      {breadcrumb ? (
        <div className="inline cursor-pointer items-center pl-1 md:pl-2 text-sm bg-slate-700 hover:bg-slate-600 active:bg-slate-500 bg-opacity-50 transition-colors duration-300">
          <span className="" onClick={onClickHandler}>
            Trash
          </span>
