...
import {
  ...
  UploadIdbType,
  UploadStateEnum,
  VideoPlaybackRowType,
} from "./collections2Store.types";
...
function createObjectStores(db: IDBPDatabase, oldVersion?: number) {
  ...
  switch (oldVersion) {
    ...
    case 1:
      // Create stores
      tuuidStore = db.createObjectStore(STORE_TUUIDS, {
        keyPath: ["tuuid", "user_id"],
      });
      db.createObjectStore(STORE_VIDEO_PLAY, { keyPath: ["tuuid", "userId"] });

      // Create indices
      tuuidStore.createIndex("parent", ["parent", "user_id"], {
        unique: false,
        multiEntry: false,
      });
    ...
    case 2:
      downloadStore = db.createObjectStore(STORE_DOWNLOADS, {
        keyPath: ["fuuid", "userId"],
      });
      db.createObjectStore(STORE_DOWNLOAD_PARTS, {
        keyPath: ["fuuid", "position"],
      });

      // Create indices
      downloadStore.createIndex("tuuid", ["tuuid", "userId"], {
        unique: false,
        multiEntry: false,
      });
      downloadStore.createIndex("state", ["userId", "state", "processDate"], {
        unique: false,
        multiEntry: false,
      });
    ...
    // @ts-ignore Fallthrough
    case 3:
      uploadStore = db.createObjectStore(STORE_UPLOADS, {
        keyPath: "uploadId",
        autoIncrement: true,
      });
      db.createObjectStore(STORE_UPLOAD_PARTS, {
        keyPath: ["uploadId", "position"],
      });

      // Create indices
      uploadStore.createIndex("state", ["userId", "state", "processDate"], {
        unique: false,
        multiEntry: false,
      });
    ...
    // @ts-ignore Fallthrough
    case 4: // Most recent
      db.createObjectStore(STORE_DOWNLOAD_DECRYPTED_PARTS, {
        keyPath: ["fuuid", "position"],
      });
      break;
...
export async function loadTuuid(
  tuuid: string,
  userId: string,
): Promise<TuuidsIdbStoreRowType | null> {
  const db = await openDB();
...
export async function loadDirectory(
  userId: string,
  tuuid: string | null,
): Promise<LoadDirectoryResultType> {
  const db = await openDB();
  let store = db.transaction(STORE_TUUIDS, "readonly").store;
...
export async function touchDirectorySync(
  tuuid: string,
  userId: string,
  lastCompleteSyncSec: number,
) {
  const db = await openDB();
  const store = db.transaction(STORE_TUUIDS, "readwrite").store;
...
export async function setVideoPosition(
  tuuid: string,
  userId: string,
...
export async function getCurrentVideoPosition(
  tuuid: string,
  userId: string,
): Promise<VideoPlaybackRowType | null> {
  const db = await openDB();
  const store = db.transaction(STORE_VIDEO_PLAY, "readonly").store;
  return await store.get([tuuid, userId]);
}

export async function getDownloadContent(
  fuuid: string,
  userId?: string,
): Promise<Blob | null> {
  // const db = await openDB();
  ...
  try {
    ...
    try {
      let fileHandle = await downloadDirectory.getFileHandle(
        `${fuuid}.decrypted`,
      );
...
export async function removeDownloadParts(fuuid: string) {
  ...
  storeParts.delete(
    IDBKeyRange.bound([fuuid, 0], [fuuid, Number.MAX_SAFE_INTEGER]),
  );
  const storeDecryptedParts = db.transaction(
    STORE_DOWNLOAD_DECRYPTED_PARTS,
    "readwrite",
  ).store;
  storeDecryptedParts.delete(
    IDBKeyRange.bound([fuuid, 0], [fuuid, Number.MAX_SAFE_INTEGER]),
  );
...
export async function getNextDownloadJob(
  ...
  let job = await stateIndex.getAll(
    IDBKeyRange.bound(
      [userId, DownloadStateEnum.INITIAL, 0],
      [userId, DownloadStateEnum.INITIAL, Number.MAX_SAFE_INTEGER],
    ),
    1,
  );
 ...
 * @returns
 */
export async function restartNextJobInError(
  userId: string,
): Promise<DownloadIdbType | null> {
  const db = await openDB();
  const store = db.transaction(STORE_DOWNLOADS, "readwrite").store;
  ...
  let cursor = await stateIndex.openCursor(
    IDBKeyRange.bound(
      [userId, DownloadStateEnum.ERROR, 0],
      [userId, DownloadStateEnum.ERROR, Number.MAX_SAFE_INTEGER],
    ),
  );
...
export async function getNextDecryptionJob(
  userId: string,
): Promise<DownloadIdbType | null> {
  const db = await openDB();
  const store = db.transaction(STORE_DOWNLOADS, "readonly").store;
  ...
  let job = await stateIndex.getAll(
    IDBKeyRange.bound(
      [userId, DownloadStateEnum.ENCRYPTED, 0],
      [userId, DownloadStateEnum.ENCRYPTED, Number.MAX_SAFE_INTEGER],
    ),
    1,
  );
...
export async function updateDownloadJobState(
  fuuid: string,
  userId: string,
  ...
  if (!job)
    throw new Error(`Download job fuuid:${fuuid} userId:${userId} not found`);
...
export async function saveDownloadPart(
  fuuid: string,
  position: number,
  part: Blob,
) {
...
export async function saveDownloadDecryptedPart(
  fuuid: string,
  position: number,
  part: Blob,
) {
...
export async function saveDecryptionError(fuuid: string) {
  ...
  await storeDecryptedParts.delete(
    IDBKeyRange.bound([fuuid, 0], [fuuid, Number.MAX_SAFE_INTEGER]),
  );
 ...
 */
export async function findDownloadPosition(
  fuuid: string,
): Promise<number | null> {
  let root = await navigator.storage.getDirectory();
  try {
...
export async function removeUserDownloads(
  userId: string,
  opts?: { state?: DownloadStateEnum; fuuid?: string },
) {
  const db = await openDB();
  ...
  if (opts?.fuuid) {
    ...
    let cursorUserJobs = await stateIndex.openCursor(
      IDBKeyRange.bound(
        [userId, state, 0],
        [userId, state, Number.MAX_SAFE_INTEGER],
      ),
    );
  ...
  if (fuuids.size > 0) {
    ...
    let storeDownloadParts = db.transaction(
      STORE_DOWNLOAD_PARTS,
      "readwrite",
    ).store;
    ...
    fuuids.forEach((fuuid) => {
      removeDownloadStorageEntry(fuuid as string).catch((err) =>
        console.error("Error deleting downloaded file %s: %O", fuuid, err),
      );
    });
  }
}
...
export async function getUploadJobs(userId: string): Promise<UploadJobType[]> {
  ...
  let jobs = (await stateIndex.getAll(
    IDBKeyRange.bound(
      [userId, UploadStateEnum.INITIAL, 0],
      [userId, UploadStateEnum.ERROR, Number.MAX_SAFE_INTEGER],
    ),
  )) as UploadJobType[];
  return jobs;
}
...
export async function getUploadJob(
  uploadId: number,
): Promise<UploadJobType | null> {
  const db = await openDB();
  const store = db.transaction(STORE_UPLOADS, "readonly").store;
  return await store.get(uploadId);
}

export async function getNextUploadSendCommand(
  userId: string,
  retry?: boolean,
): Promise<UploadIdbType | null> {
  const db = await openDB();
  const store = db.transaction(STORE_UPLOADS, "readwrite").store;
  let stateIndex = store.index("state");
  let cursor = await stateIndex.openCursor(
    IDBKeyRange.bound(
      [userId, UploadStateEnum.SENDCOMMAND, 0],
      [userId, UploadStateEnum.SENDCOMMAND, Number.MAX_SAFE_INTEGER],
    ),
  );
  while (cursor) {
...
export async function getBatchRetryUploadSendCommand(
  userId: string,
): Promise<UploadIdbType[] | null> {
  const db = await openDB();
  const store = db.transaction(STORE_UPLOADS, "readwrite").store;
...
export async function updateUploadJobState(
  uploadId: number,
  state: UploadStateEnum,
  opts?: { uploadUrl?: string },
) {
  const db = await openDB();
  const store = db.transaction(STORE_UPLOADS, "readwrite").store;
...
export async function removeUploadParts(uploadId: number) {
  ...
  storeParts.delete(
    IDBKeyRange.bound([uploadId, 0], [uploadId, Number.MAX_SAFE_INTEGER]),
  );
}
...
export async function getUploadPart(
  uploadId: number,
  position: number,
): Promise<UploadIdbParts | null> {
  const db = await openDB();
  const store = db.transaction(STORE_UPLOAD_PARTS, "readonly").store;
  return await store.get([uploadId, position]);
}

export async function saveUploadJobDecryptionInfo(
  uploadId: number,
  decryptionInfo: messageStruct.MessageDecryption,
  fileSize: number,
  ...
  if (decryptionInfo.verification) {
    ...
    throw new Error(
      "File unique id (fuuid) not available from encrypted information",
    );
  }
...
export async function saveUploadJobAddCommand(
  uploadId: number,
  command: Collections2AddFileCommand,
) {
  const db = await openDB();
  const store = db.transaction(STORE_UPLOADS, "readwrite").store;
  let job = (await store.get(uploadId)) as UploadIdbType | null;
...
export async function getNextUploadReadyJob(
  userId: string,
): Promise<UploadJobType | null> {
  const db = await openDB();
  const store = db.transaction(STORE_UPLOADS, "readonly").store;
  ...
  let job = await stateIndex.getAll(
    IDBKeyRange.bound(
      [userId, UploadStateEnum.READY, 0],
      [userId, UploadStateEnum.READY, Number.MAX_SAFE_INTEGER],
    ),
    1,
  );
...
export async function removeUserUploads(
  userId: string,
  opts?: { state?: UploadStateEnum; uploadId?: number },
) {
  const db = await openDB();
...
export async function cleanup() {
  ...
  let storeDownloadDecryptedParts = db.transaction(
    STORE_DOWNLOAD_DECRYPTED_PARTS,
    "readwrite",
  ).store;
