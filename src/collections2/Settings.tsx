...
function ResolutionSettings() {
  ...
  let optionsElem = useMemo(() => {
    return VIDEO_RESOLUTIONS.map((item) => {
      return (
        <option key={item.value} value={item.value} className="font-black">
          {item.label}
        </option>
      );
    });
  ...
  return (
    <div className="grid col-span-1 w-64 pt-4">
      <label htmlFor="select-resolution">
      ...
      <div>
        <ActionButton onClick={actionHandler} revertSuccessTimeout={3}>
          Change
        </ActionButton>
      </div>
    </div>
...
function FilehostConfiguration() {
  ...
  useEffect(() => {
    ...
    workers.connection.getFilehosts().then((response) => {
  ...
  return (
    <>
      <p>
        You can select the filehost to use for uploading and downloading files.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 pt-2">
        ...
        <select
          ...
          className="col-span-2 text-black bg-slate-300"
        >
      ...
      <ActionButton onClick={changeFilehostHandler} disabled={!ready}>
        Save
      </ActionButton>
    </>
  );
...
function Cleanup() {
  ...
  useEffect(() => {
    ...
    if (navigator.storage.estimate) {
      loadEstimate(setStorageUsage).catch((err) =>
  ...
  return (
    <div className="pt-4">
      <p>
        Cleanup history - removes all downloaded files and decrypted content
        from Collections2. Maintains configuration.
      </p>
      {typeof storageUsage === "number" ? (
        <>
          <p className="pt-2">Current estimated usage for MilleGrilles:</p>
          <p className="font-bold">
            <Formatters.FormatteurTaille value={storageUsage} />
          </p>
        </>
      ) : (
        <></>
...
function StoragePersistence() {
  ...
  return (
    <>
      ...
      <p>
        This is useful to process larger files - it increases the amount of disk
        space the browser can use.
      </p>
      <p>Storage estimate:</p>
      {storageElem}
      <ActionButton onClick={persistenceCallback} disabled={persistenceState}>
        Allow disk usage
      </ActionButton>
    </>
  );
...
function TestArea() {
  ...
  let downloadWorkerCallback = useCallback(async () => {
    ...
    await testBounds(
      "zSEfXUA2auCgqVJ1Lrxv9yF9vgq9se7CDGhMdFNynV43EAitCuZuAXMsyCNz75uiKG3ibgrvkdLP4WHtjAxrmZMtM6k4Ji",
    );
  }, [workers, ready]);
  ...
  let fsListDownloads = useCallback(async () => {
    if (!navigator.storage.getDirectory)
      throw new Error("getDirectory not supported");
  ...
  return (
    <>
      ...
      <ActionButton onClick={downloadWorkerCallback}>
        Download test
      </ActionButton>
      <p>File system</p>
