...
import {
  Collection2DirectoryContentUpdateMessage,
  Collection2DirectoryStats,
  Collection2DirectoryUpdateMessage,
...
function ViewUserFileBrowsing() {
  ...
  const setSelectionMode = useUserBrowsingStore(
    (state) => state.setSelectionMode,
  );
  const setSelectionPosition = useUserBrowsingStore(
    (state) => state.setSelectionPosition,
  );
  const setBrowsingSort = useUserBrowsingStore(
    (state) => state.setBrowsingSort,
  );
...
export function DirectorySyncHandler(props: {
  ...
  let updateCurrentDirectory = useUserBrowsingStore(
    (state) => state.updateCurrentDirectory,
  );
  ...
  let setDirectoryStatistics = useUserBrowsingStore(
    (state) => state.setDirectoryStatistics,
  );
  ...
  let directoryUpdateHandler = useCallback(
  ...
  let directoryUpdateProxy = useMemo(
    () => proxy(directoryUpdateHandler),
    [directoryUpdateHandler],
  );

  let directoryContentUpdateHandler = useCallback(
    (e: SubscriptionMessage) => {
      if (!workers || !userId) {
        console.warn(
          "Subscription message received when workers/userId is not initialized, ignored",
      ...
      updateCollectionContent(
        ...
        console.error("Error handling directory content update", err),
      );
    },
    [workers, userId, updateCurrentDirectoryHandler, deleteFilesDirectory],
  );
  let directoryContentUpdateProxy = useMemo(
    () => proxy(directoryContentUpdateHandler),
    [directoryContentUpdateHandler],
  );
  ...
  useEffect(() => {
    ...
    Promise.resolve()
      .then(async () => {
        ...
        await workers.connection.collection2SubscribeCollectionEvents(
          listenerParam,
          directoryUpdateProxy,
        );
        await workers.connection.collection2SubscribeCollectionContentEvents(
          listenerParam,
          directoryContentUpdateProxy,
        );
      })
      .catch((err) =>
        console.error(
          "Error registering directory listener on %s: %O",
          listenerParam,
          err,
        ),
      );
    ...
    return () => {
      ...
      Promise.resolve()
        .then(async () => {
          ...
          await workers.connection.collection2UnsubscribeCollectionEvents(
            listenerParam,
            directoryUpdateProxy,
          );
          await workers.connection.collection2UnsubscribeCollectionContentEvents(
            listenerParam,
            directoryContentUpdateProxy,
          );
        })
        .catch((err) =>
          console.error(
            "Error unregistering directory listener on %s: %O",
            tuuid,
            err,
          ),
        );
...
async function synchronizeDirectory(
  ...
  setBreadcrumb: (
    username: string,
    dirs: TuuidsBrowsingStoreRow[] | null,
  ) => void,
  setDirectoryStatistics: (
  ...
  while (!complete) {
    ...
    const response = await workers.connection.syncDirectory(
      tuuid,
      skip,
      syncDate,
    );
