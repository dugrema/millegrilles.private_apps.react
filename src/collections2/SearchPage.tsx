...
import useUserBrowsingStore, {
  Collection2SearchStore,
  filesIdbToBrowsing,
  TuuidsBrowsingStoreRow,
  TuuidsBrowsingStoreSearchRow,
} from "./userBrowsingStore";
import {
  Collection2SearchResultsContent,
  Collection2SearchResultsDoc,
...
function SearchPage() {
  ...
  const setSearchResults = useUserBrowsingStore(
    (state) => state.setSearchResults,
  );
  const searchResultsPosition = useUserBrowsingStore(
    (state) => state.searchResultsPosition,
  );
  const setSearchResultsPosition = useUserBrowsingStore(
    (state) => state.setSearchResultsPosition,
  );
  ...
  const searchScopeOnChange = useCallback(
  ...
  const searchTypeOnChange = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => setSearchType(e.currentTarget.value),
    [setSearchType],
  );
  ...
  const queryRagHandler = useCallback(async () => {
    ...
    if (searchInput) {
      const searchInputDict = {
        query: searchInput,
        cuuid: null as string | null,
      };
      if (cuuid && searchScope === "directory") {
        searchInputDict.cuuid = cuuid;
      }
      console.debug(
        "Search params (scope: %O): %O",
        searchScope,
        searchInputDict,
      );
      const encryptedMessage =
        await workers.encryption.encryptMessageMgs4ForDomain(
          searchInputDict,
          "ollama_relai",
        );
      ...
      if (response.ok !== true)
        throw new Error("Error during RAG query: " + response.err);
        ...
        <form onSubmit={submitHandler}>
          <div className="grid grid-cols-6 sm:grid-cols-12">
            ...
            <ActionButton
              ...
              className="ml-1 text-center col-span-2 lg:col-span-1"
              mainButton={true}
            >
              Search
            </ActionButton>
...
function SearchResultSection(props: {
  ...
  const [list, setList] = useState(
    null as TuuidsBrowsingStoreSearchRow[] | null,
  );
  const [sharedCuuids, setSharedCuuids] = useState(
    null as {
      [tuuid: string]: Collections2SharedContactsSharedCollection;
    } | null,
  );
  ...
  useEffect(() => {
    ...
    workers.connection
      .getCollections2SharedContactsWithUser()
      .then((response) => {
        if (response.ok === false) throw new Error(response.err);
        if (response.partages) {
          let sharesByTuuid = response.partages.reduce(
            (acc, item) => {
  ...
  useEffect(() => {
    ...
    parseSearchResults(workers, userId, sharedCuuids, data, page).then(
      (results) => {
  ...
  return (
    <section
      ref={navSectionRef}
      className="fixed top-32 left-0 px-2 bottom-10 overflow-y-auto w-full"
    >
...
function useSearchResults(): UseSearchResultsType {
  ...
  const [fetcherKey, fetcherFunction] = useMemo(() => {
    ...
    if (!workers || !ready || !searchParams || !workers || !query)
      return [[null, null], null];
    const fetcherFunction = async (params: [string, string | null]) =>
      workers?.connection.searchFiles(params[0], params[1], CONST_PAGE_SIZE);
    return [[query, cuuid], fetcherFunction];
  }, [workers, ready, searchParams]);

  const { data, error, isLoading } = useSWR(fetcherKey, fetcherFunction);

  if (searchResults?.ragResponse)
    return {
...
async function parseSearchResults(
  ...
  data: Collections2SearchResults | null,
  page: number,
): Promise<TuuidsBrowsingStoreSearchRow[] | null> {
  ...
  if (!files) {
    ...
    const response = await workers.connection.getFilesByTuuid(tuuids, {
      shared: true,
    });
  ...
  const decryptedFiles = await loadFileData(
    workers,
    userId,
    sharedCuuids,
    pageDocs,
...
async function loadFileData(
  ...
  docs: Collection2SearchResultsDoc[],
  files: Collections2FileSyncRow[],
  keys: DecryptedSecretKey[],
): Promise<TuuidsBrowsingStoreSearchRow[] | null> {
  ...
  let decryptedFiles = await workers.directory.processDirectoryChunk(
    workers.encryption,
    userId,
    files,
    keys,
  ...
  let storeFilesByTuuid = storeFiles.reduce(
    (acc, item) => {
      acc[item.tuuid] = item;
