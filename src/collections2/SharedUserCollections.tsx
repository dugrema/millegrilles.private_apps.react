...
import { Collection2SharedCollection } from "../types/connection.types";
...
function SharedUserCollections() {
  ...
  let [shares, setShares] = useState(
    null as Collection2SharedCollection[] | null,
  );
  ...
  let deleteHandler = useCallback(
    async (e: MouseEvent<HTMLButtonElement>) => {
      ...
      if (item && item.contact_id) {
        workers.connection
          .removeShareCollection2Collection(item.tuuid, item.contact_id)
          .then((response) => {
            if (response.ok === false)
              throw new Error(
                "Error deleting shared collection: " + response.err,
              );
            if (collections) {
              let udpatedCollections = collections.filter(
                (item) => item.tuuid !== value,
              );
  ...
  useEffect(() => {
    ...
    Promise.resolve()
      .then(async () => {
        ...
        let contacts = contactsResponse.contacts.filter(
          (item) => item.user_id === sharedUserId,
        );
        let username = "";
        ...
        let shares = sharesResponse.partages.filter((item) =>
          contactIds.has(item.contact_id),
        );
        ...
        if (shares.length > 0) {
          ...
          let sharedCollections =
            await workers.connection.getFilesByTuuid(tuuids);
          if (sharedCollections.ok === false)
            throw new Error(
              "Error loading shared folders: " + sharedCollections.err,
            );
          if (!sharedCollections.files)
            throw new Error("Error loading shared folders: no files returned");
          let collections = await workers.directory.processDirectoryChunk(
            workers.encryption,
            userId,
            sharedCollections.files,
            sharedCollections.keys,
          );
          console.debug("Shared collections: ", collections);
    ...
    <>
      <section className="fixed top-12">
        <h1 className="pt-2 pb-2 text-xl font-bold">
          Collections shared with {username}
        </h1>
      </section>
...
function DisplayCollections(props: {
  ...
  let elems = value.map((item) => {
    return (
      <Link
        key={item.tuuid}
        to={`/apps/collections2/b/${item.tuuid}`}
