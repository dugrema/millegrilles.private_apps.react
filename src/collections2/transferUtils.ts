...
import { addUploadFile, loadTuuid } from "./idb/collections2StoreIdb";
 ...
 * @param userId
 */
export async function createDownloadEntryFromFile(
  tuuid: string,
  userId: string,
  ...
  // Content validation
  if (!file)
    throw new Error(`Unkonwn file tuuid: ${tuuid} for userId: ${userId}`);
  if (file.type_node !== "Fichier")
    throw new Error(
      `File tuuid ${tuuid} is of type ${file.type_node}. Only 'Fichiers' type can be downloaded`,
    );
  ...
  if (typeof fileSize !== "number")
    throw new Error(`The size for file ${tuuid} was not provided.`);

  let visits = file.fileData.visites;
  if (!visits || Object.keys(visits).length === 0)
    throw new Error(
      `There are no known locations to download file tuuid ${tuuid}`,
    );

  let fuuids = file.fileData.fuuids_versions;
  if (!fuuids || fuuids.length === 0)
    throw new Error(
      `File ${tuuid} is missing version information for download`,
    );
  ...
  if (!secretKey)
    throw new Error(`Decryption key for file tuuid ${tuuid} not found`);
  let format = file.fileData.format;
  if (!format)
    throw new Error(`Decryption format for file tuuid ${tuuid} not found`);
  let nonceBase64 = file.fileData.nonce;
  if (!nonceBase64)
    throw new Error(`Decryption nonce for file tuuid ${tuuid} not found`);
...
export async function createDownloadEntryFromVideo(
  tuuid: string,
  userId: string,
  video: FileVideoData,
): Promise<DownloadIdbType> {
  ...
  if (typeof fileSize !== "number")
    throw new Error(`The size for video of file ${tuuid} was not provided.`);
  ...
  if (!format)
    throw new Error(`Decryption format for video of ${tuuid} not found`);
  let nonceBase64 = video.nonce;
  if (!nonceBase64)
    throw new Error(`Decryption nonce for video of tuuid ${tuuid} not found`);
  let nonce = multiencoding.decodeBase64Nopad(nonceBase64);
...
export async function generateFileUploads(
  ...
  for await (let file of files) {
    ...
    let uploadId = await addUploadFile(userId, cuuid, file, {
      secret,
      keyCommand: keyCommandSigned,
      destinationPath: breadcrumb,
    });
