...
export type TuuidsBrowsingStoreSearchRow = TuuidsBrowsingStoreRow & {
  score: number;
  contactId?: string | null;
};
...
export function filesIdbToBrowsing(
  files: TuuidsIdbStoreRowType[],
): TuuidsBrowsingStoreRow[] {
  return files
    .filter((item) => item.decryptedMetadata)
    .map((item) => {
  ...
  devtools((set) => ({
    ...
    updateCurrentDirectory: (files) =>
      set((state) => {
        ...
        let currentDirectory = {} as {
          [tuuid: string]: TuuidsBrowsingStoreRow;
        };
    ...
    updateCurrentDirectoryDeleted: (files) =>
      set((state) => {
    ...
    setBreadcrumb: (username, breadcrumb) =>
      set(() => ({ usernameBreadcrumb: username, breadcrumb })),
    setDirectoryStatistics: (directoryStatistics) =>
      set(() => ({ directoryStatistics })),
    setLastOpenedFile: (lastOpenedFile) => set(() => ({ lastOpenedFile })),
    updateThumbnail: (tuuid, thumbnail) =>
      set((state) => {
    ...
    deleteFilesDirectory: (files: string[]) =>
      set((state) => {
        let updatedDirectory = {} as {
          [tuuid: string]: TuuidsBrowsingStoreRow;
        };
        if (state.currentDirectory) {
    ...
    setSearchResultsPosition: (searchResultsPosition) =>
      set(() => ({ searchResultsPosition })),

    updateSearchListing: (listing) =>
      set((state) => {
        if (!listing) {
          // Clear
        ...
        let searchListing = {} as {
          [tuuid: string]: TuuidsBrowsingStoreSearchRow;
        };
    ...
    setSelectionMode: (selectionMode) =>
      set(() => ({ selectionMode, selection: null })),
    setSelection: (selection) => set(() => ({ selection })),
    setSelectionPosition: (selectionPosition) =>
      set(() => ({ selectionPosition })),
    setBrowsingSort: (key: string, order: number) =>
      set(() => ({ browsingSort: { key, order } })),

    setSharedWithUser: (sharedWithUser) => set(() => ({ sharedWithUser })),
    setSharedContact: (sharedContact) => set(() => ({ sharedContact })),
    setSharedBreadcrumb: (sharedBreadcrumb) =>
      set(() => ({ sharedBreadcrumb })),
    setSharedDirectoryStatistics: (sharedDirectoryStatistics) =>
      set(() => ({ sharedDirectoryStatistics })),
    setSharedCollection: (sharedCollection) =>
      set(() => ({ sharedCollection })),
    setSharedCuuid: (sharedCuuid) =>
      set(() => ({ sharedCuuid, selection: null, selectionMode: false })),
    updateSharedCurrentDirectory: (files) =>
      set((state) => {
        if (!files) {
          // Clear
    ...
    updateSharedThumbnail: (tuuid, thumbnail) =>
      set((state) => {
    ...
    setModalBreadcrumb: (username, breadcrumb) =>
      set(() => ({
        modalNavUsername: username,
        modalNavBreadcrumb: breadcrumb,
      })),
    updateModalCurrentDirectory: (files) =>
      set((state) => {
        if (!files) {
          // Clear
